...
Minimises the whitened objective by alternating:
...
def gn_minimize(
    ...
    *,
    ...
    r"""Direct Gauss–Newton outer loop on the whitened Gram.

    The linear-in-θ fast path: the exact core already returns the GN
    ...
    merit : {"nll", "phi"}  line-search / best-iterate merit.  ``"nll"`` (the
        symmetric MLE) descends the banded NLL.  ``"phi"`` is the
    ...
    # Sensitivity-collapse guard.  On the ``"phi"`` (EIV / IV-root) merit the
    # objective is ‖f‖ = ‖ψᵀPr‖, which a nonlinear-in-θ model can drive to
    # zero by sending its *sensitivities* ψ → 0 (a saturating tanh MLP, an
    # alignment gain → ∞) rather than by fitting the data — a spurious root
    # far from the truth.  Its fingerprint is the symmetric Gram ‖½(G+Gᵀ)‖ =
    # ‖ψᵀPψ‖ collapsing by orders of magnitude (measured: 8e2 → 3e-7 in one
    # step for a runaway MLP; flat for a well-conditioned PSF).  The healthy
    # optimisation path is untouched (``best`` updates exactly as before —
    # golden parity); we additionally snapshot ``best_confirmed`` whenever
    # the Gram at the current θ verifies healthy, and on collapse we roll
    # ``best`` back to that snapshot (the collapsed θ has ‖f‖ ≈ 0 and would
    # otherwise win) and report ``diverged`` for the caller's fallback.
    first_gram_norm = None
    best_confirmed = (best_nll, best)
    COLLAPSE_FRAC = 1e-6
    ...
    for outer in range(max_iter):
        ...
        # Collapse detection is armed on the phi merit only; the nll (MLE /
        # linear-GN) path is untouched.
        if merit == "phi":
            gram_norm = float(jnp.linalg.norm(0.5 * (G + G.T)))
            if first_gram_norm is None and gram_norm > 0.0:
                first_gram_norm = gram_norm
            elif (first_gram_norm is not None
                    and gram_norm < COLLAPSE_FRAC * first_gram_norm):
                # ψ has collapsed — the current θ (and any best recorded on
                # its way here) is the spurious root: roll back to the last
                # Gram-verified iterate and hand control to the caller.
                diverged = True
                best_nll, best = best_confirmed
                break
            # the Gram at the CURRENT θ verifies healthy: everything recorded
            # in ``best`` up to now is trustworthy.
            best_confirmed = (best_nll, best)
        ...
        while not accepted and retries <= 8:
            ...
            for alpha in line_search_alphas:
                ...
                if nll_trial < nll_val + 1e-8:
                    theta = theta_trial
                    accepted = True
                    # Original semantics (golden parity): a collapsed phi
                    # step recorded here is rolled back to ``best_confirmed``
                    # by the guard at the next iteration's Gram check.
    ...
    # Tail guard (phi only): a collapse step accepted at the very last
    # iteration escapes the top-of-loop check — verify the returned iterate's
    # Gram once and roll back if it is the spurious root.
    if (merit == "phi" and not diverged and first_gram_norm is not None
            and best is not best_confirmed[1]):
        G_b, _, _ = gram_fn(best[0], best[1], best[2])
        if (float(jnp.linalg.norm(0.5 * (G_b + G_b.T)))
                < COLLAPSE_FRAC * first_gram_norm):
            diverged = True
            best_nll, best = best_confirmed

    info = {"outer_iterations": outer + 1, "converged": converged,
            "diverged": diverged, "loss": best_nll}
    return best[0], best[1], best[2], info
