...
"""
Flow-composed per-particle Jacobians and θ-sensitivities.

Originally the interacting-path machinery (moved verbatim from the legacy
``_od_precompute`` / ``_underdamped``) — the frozen-background
approximation: the state update uses the full multi-particle force, while
the tangent is propagated per particle from the same-particle
derivatives, keeping the flow Jacobian block-diagonal in the particle
axis and the cost O(N) per window.

For **non-interacting** models (``particles_input=False``) there are no
cross-particle terms, so the same recursions are the *exact* chain rule —
and replace forward-mode AD with n_params tangents through the flow by
one force/derivative evaluation per stage (see ``flow_multi._jpb``).

Both ``integrator="rk4"`` and ``"euler"`` substeps are supported; the
Euler tangents are ``J_step = I + h·A₁`` and ``B_step = h·Θ₁``.
"""
...
__all__ = ["rk4_composed_jacobian", "rk4_composed_jacobian_theta",
           ...
           "lyapunov_from_stages"]


def _check_integrator(integrator, fname):
    if integrator not in ("rk4", "euler"):
        raise ValueError(
            f"{fname} supports integrator='rk4' or 'euler', got {integrator!r}"
        )
...
def _lyap_rk4_update(A1, A2, A3, A4, Q, h, twoD):
    """One RK4 substep of ``Q̇ = A(t)Q + QA(t)ᵀ + twoD`` on cached stages.

    Shared by the fused in-scan accumulation and
    :func:`lyapunov_from_stages`, so the two paths are
    arithmetic-identical."""
    def rate(A, Qv):
        return _bmm(A, Qv) + _bmm(Qv, jnp.swapaxes(A, -1, -2)) + twoD

    q1 = rate(A1, Q)
    q2 = rate(A2, Q + (h / 2) * q1)
    q3 = rate(A3, Q + (h / 2) * q2)
    q4 = rate(A4, Q + h * q3)
    return Q + (h / 6) * (q1 + 2 * q2 + 2 * q3 + q4)


def _lyap_euler_update(A1, Q, h, twoD):
    return Q + h * (_bmm(A1, Q) + _bmm(Q, jnp.swapaxes(A1, -1, -2)) + twoD)


def lyapunov_from_stages(stages, h, twoD, *, integrator="rk4"):
    r"""Process covariance ``Q`` from *cached* stage Jacobians.

    ``stages`` is the per-interval stack emitted by the θ-recursions with
    ``return_stages=True``: shape ``(n_substeps, S, ..., m, m)`` with
    ``S = 4`` (RK4 stage matrices) or ``S = 1`` (Euler).  Re-runs the same
    per-substep update as the fused in-scan path (identical arithmetic),
    which makes ``Q`` recomputable for a new noise intensity ``twoD``
    (a ``(m, m)`` matrix, or batched against the trailing stage axes for
    state-dependent diffusion) with **zero force/basis evaluations** —
    the (D, Λ) profile and diffusion-solve fast path.
    """
    _check_integrator(integrator, "lyapunov_from_stages")
    Q0 = jnp.zeros(stages.shape[2:], dtype=stages.dtype)
    if twoD is not None:
        Q0 = jnp.zeros(jnp.broadcast_shapes(Q0.shape, jnp.shape(twoD)),
                       dtype=stages.dtype)

    if integrator == "rk4":
        def step(Q, A):
            return _lyap_rk4_update(A[0], A[1], A[2], A[3], Q, h, twoD), None
    else:
        def step(Q, A):
            return _lyap_euler_update(A[0], Q, h, twoD), None

    Q, _ = lax.scan(step, Q0, stages)
    return Q
...
def rk4_composed_jacobian_theta(F_psf, dFdx_expr, dFdth_fn, n_params, X_start, theta_struct,
    ...
    *fixed* stage frame (exact — the force's direct θ-dependence is
    ...
    D_lyap : ``(d, d)`` or None
        When given, additionally integrate the Lyapunov ODE
        ``Q̇ = A(t) Q + Q A(t)ᵀ + 2 D_lyap`` (``Q(0) = 0``) along the same
        stages — the exact linearized process-noise covariance
        ``Q = ∫ U(Δt,s)\,2D\,U(Δt,s)ᵀ ds`` to integrator order, replacing
        the endpoint-trapezoid ``Δt (J D Jᵀ + D)``.
    Other parameters as in :func:`rk4_composed_jacobian`.

    Returns
    -------
    J : ``(N, d, d)``,  Phi : ``(N, d)``,  B : ``(N, d, n_params)``
    [, Q : ``(N, d, d)`` when ``D_lyap`` is given]
    [, stages : ``(n_substeps, S, N, d, d)`` when ``return_stages`` —
    the per-stage Jacobians consumed by :func:`lyapunov_from_stages`]
    """
    _check_integrator(integrator, "rk4_composed_jacobian_theta")
    h = dt / n_substeps
    d = X_start.shape[-1]
    I_d = jnp.eye(d)
    with_q = D_lyap is not None
    twoD = None if not with_q else (D_lyap + D_lyap.T)
    ...
    def _euler_step(carry, _):
        X, J_acc, B_acc, Q_acc = carry
        k1 = _force(X)
        A1 = _dFdx(X)
        J_step = I_d + h * A1
        B_step = h * dFdth_fn(X)
        Q_new = Q_acc if not with_q else _lyap_euler_update(A1, Q_acc, h, twoD)
        ys = A1[None] if return_stages else None
        return (X + h * k1, _mm(J_step, J_acc), _mm(J_step, B_acc) + B_step,
                Q_new), ys
    ...
    def _rk4_step(carry, _):
        ...
        if with_q:
            # RK4 on the (matrix) Lyapunov ODE, stage A's reused
            Q_new = _lyap_rk4_update(A1, A2, A3, A4, Q_acc, h, twoD)
        else:
            Q_new = Q_acc
        ys = jnp.stack([A1, A2, A3, A4]) if return_stages else None
        return (X_new, J_acc_new, B_acc_new, Q_new), ys

    step = _rk4_step if integrator == "rk4" else _euler_step
    J0 = jnp.broadcast_to(I_d, X_start.shape[:-1] + (d, d))
    B0 = jnp.zeros(X_start.shape[:-1] + (d, n_params), dtype=X_start.dtype)
    Q0 = jnp.zeros(X_start.shape[:-1] + (d, d), dtype=X_start.dtype)
    (X_final, J_composed, B_composed, Q_composed), stages = lax.scan(
        step, (X_start, J0, B0, Q0), None, length=n_substeps)
    out = (J_composed, X_final - X_start, B_composed)
    if with_q:
        out = out + (Q_composed,)
    if return_stages:
        out = out + (stages,)
...
def rk4_composed_jacobian(F_psf, dFdx_expr, X_start, theta_struct, mask, extras, dt, n_substeps, integrator="rk4"):
    ...
    dt : float
    n_substeps : int
    integrator : {"rk4", "euler"}, optional
        Substep scheme; the substep Jacobian is the matching hand-rolled
        chain rule below.
    ...
    """
    _check_integrator(integrator, "rk4_composed_jacobian")
    h = dt / n_substeps
    d = X_start.shape[-1]
    I_d = jnp.eye(d)
    ...
    def _euler_step(carry, _):
        X, J_acc = carry
        k1 = _force(X)
        J_step = I_d + h * _dFdx(X)
        return (X + h * k1, _mm(J_step, J_acc)), None
...
def rk4_composed_jacobian_phase(
    ...
    """
    _check_integrator(integrator, "rk4_composed_jacobian_phase")
    ...
    def _euler_step(carry, _):
        X, V, J_acc = carry
        k1_v = _force(X, V)
        M1 = _M_stack(_dFdx(X, V), _dFdv(X, V))
        J_step = I_2d + h * M1
        return (X + h * V, V + h * k1_v, _mm(J_step, J_acc)), None
...
def rk4_composed_jacobian_phase_theta(
    ...
    [, Q : ``(N, 2d, 2d)`` lifted process covariance when ``D_lyap`` given:
    the solution of ``Q̇ = M Q + Q Mᵀ + diag(0, 2 D_lyap)`` along the
    stages — the exact linearized phase-space noise accrued over ``dt``.]
    [, stages : ``(n_substeps, S, N, 2d, 2d)`` when ``return_stages`` —
    the lifted stage matrices consumed by :func:`lyapunov_from_stages`
    with ``twoD = diag(0, 2D)``.]
    """
    _check_integrator(integrator, "rk4_composed_jacobian_phase_theta")
    import jax.lax as lax
    ...
    with_q = D_lyap is not None
    if with_q:
        twoD = D_lyap + D_lyap.T
        B_noise = jnp.zeros((2 * d, 2 * d), dtype=X_start.dtype)
        B_noise = B_noise.at[d:, d:].set(twoD)

    def _euler_step(carry, _):
        X, V, J_acc, B_acc, Q_acc = carry
        k1_v = _force(X, V)
        M1 = _M_stack(_dFdx(X, V), _dFdv(X, V))
        J_step = I_2d + h * M1
        B_step = h * _lift(dFdth_fn(X, V))
        Q_new = Q_acc if not with_q else _lyap_euler_update(M1, Q_acc, h, B_noise)
        ys = M1[None] if return_stages else None
        return (X + h * V, V + h * k1_v,
                _mm(J_step, J_acc), _mm(J_step, B_acc) + B_step, Q_new), ys
    ...
    def _rk4_step(carry, _):
        ...
        if with_q:
            Q_new = _lyap_rk4_update(M1, M2, M3, M4, Q_acc, h, B_noise)
        else:
            Q_new = Q_acc
        ys = jnp.stack([M1, M2, M3, M4]) if return_stages else None
        return (X_new, V_new, J_acc_new, B_acc_new, Q_new), ys

    step = _rk4_step if integrator == "rk4" else _euler_step
    J0 = jnp.broadcast_to(I_2d, (N, 2 * d, 2 * d))
    B0 = jnp.zeros((N, 2 * d, n_params), dtype=X_start.dtype)
    Q0 = jnp.zeros((N, 2 * d, 2 * d), dtype=X_start.dtype)
    (X_final, V_final, J_composed, B_composed, Q_composed), stages = lax.scan(
        step, (X_start, V_start, J0, B0, Q0), None, length=n_substeps)
    out = (
        X_final, V_final,
