...
class UnderdampedProcess(LangevinBase):
    ...
    def simulate(
        ...
        *,
        ...
        r"""Run the integrator and return a :class:`TrajectoryCollection` of positions.
        ...
        compute_observables
            If True, accumulate the information and entropy-production
            observables along the run using the **true internal velocities**
            (which live in the integrator carry even though they are not
            recorded), and store them in the dataset metadata under the
            ``"observables"`` key.  Costs three extra force evaluations per
            substep.

        .. physics:: Information functional & entropy production (underdamped)
           :label: info-entropy-underdamped
           :category: Observable

           With :math:`F^{\pm}(x,v) = \tfrac12[F(x,v) \pm F(x,-v)]` the
           time-reversal even/odd parts of the acceleration field
           (:math:`x` even, :math:`v` odd),

           .. math::

              I \approx \tfrac{1}{4}\sum_t
                \mathrm{d}v_t^\top\, D^{-1}\, F(x_t, v_t)

           .. math::

              S \approx \sum_t
                \bigl[\mathrm{d}v_t - \bar F^{+}_t\,\mathrm{d}t\bigr]^\top
                D^{-1}\, \bar F^{-}_t ,
              \qquad
              \bar F^{\pm}_t = \tfrac12\bigl[F^{\pm}(x_t,v_t)
                + F^{\pm}(x_{t+1},v_{t+1})\bigr]

           :math:`S` is the Stratonovich log path-probability ratio
           (trajectory vs its time reverse): the entropy produced along the
           run, in units of :math:`k_B`.  The trapezoid (Stratonovich)
           evaluation of :math:`\bar F^{\pm}` is essential: it realises the
           exact cancellation
           :math:`\langle F^-\!\cdot D^{-1} F^-\rangle
           + \langle \nabla_v\!\cdot F^-\rangle = 0` at equilibrium.
        ...
        step = self._make_step(static_extras, compute_observables=compute_observables)
        if jit_compile:
            step = jit(step)

        # Carry is (x, v) — plus (I, S) accumulators when observables are on —
        # but we record positions only.
        if compute_observables:
            zero = jnp.zeros((), dtype=self._x.dtype)
            initial_state = (self._x, self._v, zero, zero)

            def _reset_observables(st):
                z = jnp.zeros((), dtype=st[2].dtype)
                return (st[0], st[1], z, z)

        else:
            initial_state = (self._x, self._v)
            _reset_observables = None
        ...
        traj_x, final_state, info = self._scan_positions(
            ...
            reset_state_fn=_reset_observables,
        )

        self._x, self._v = final_state[0], final_state[1]
        ...
        if compute_observables:
            run_meta["observables"] = {
                "information": float(final_state[2]),
                "entropy": float(final_state[3]),
            }
    ...
    # ----------------------------- Internals -----------------------------
    def _make_step(self, extras=None, *, compute_observables: bool = False):
        ...
        def _advance(x, v, ddt: float, key: Array, ex):
            ...
            return x_new, v_new, a0

        def _Dinv_at(x, v, ex) -> Array:
            # D = ½ B Bᵀ (B = sqrt(2D)); pinv tolerates singular D.
            B = _eval_B(x, v, ex)
            if B.ndim > 2:
                return jnp.linalg.pinv(0.5 * jnp.einsum("...mk,...nk->...mn", B, B))
            return jnp.linalg.pinv(0.5 * B @ B.T)

        def _contract(a: Array, Dinv: Array, b: Array) -> Array:
            # aᵀ · D⁻¹ · b summed over components and (if present) particles.
            if Dinv.ndim > 2:
                return jnp.einsum("...m,...mn,...n->", a, Dinv, b)
            return jnp.einsum("...m,mn,...n->", a, Dinv, b)
        ...
        if not compute_observables:
            ...
            def step(state, ddt: float, key: Array, sched=None):
                ...
                x_new, v_new, _ = _advance(x, v, ddt, key, ex)
                return (x_new, v_new)

            return step

        def step_obs(state, ddt: float, key: Array, sched=None):
            x, v, acc_I, acc_S = state
            ex = static_extras if sched is None else {**(static_extras or {}), **sched}
            x_new, v_new, a0 = _advance(x, v, ddt, key, ex)

            # Time-reversal parity split of the true acceleration field,
            # trapezoid (Stratonovich) evaluation over the substep — essential
            # for the equilibrium cancellation (see the physics block above).
            F_flip0 = F(x, v=-v, extras=ex)
            F_new = F(x_new, v=v_new, extras=ex)
            F_flip1 = F(x_new, v=-v_new, extras=ex)
            F_even_avg = 0.25 * (a0 + F_flip0 + F_new + F_flip1)
            F_odd_avg = 0.25 * (a0 - F_flip0 + F_new - F_flip1)

            dv = v_new - v
            x_mid = 0.5 * (x + x_new)
            v_mid = 0.5 * (v + v_new)
            Dinv = _Dinv_at(x_mid, v_mid, ex)

            acc_S = acc_S + _contract(dv - F_even_avg * ddt, Dinv, F_odd_avg)
            acc_I = acc_I + 0.25 * _contract(dv, Dinv, a0)

            return (x_new, v_new, acc_I, acc_S)
    ...
    def _scan_positions(
        ...
        *,
        ...
        :meth:`LangevinBase._scan`.  ``reset_state_fn`` (optional) is applied
        to the carry state once after the pre-run — used to zero observable
        accumulators so they cover the recorded window only.
        """
        if oversampling < 1:
