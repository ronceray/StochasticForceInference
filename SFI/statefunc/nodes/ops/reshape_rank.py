...
class ReshapeRankNode(BaseOpNode):
    ...
    * For unfolding, ``n_features`` must be divisible by ``dim^Δrank``.
    ...
    def with_children(self, new_children):
        # Preserve the target rank so StateExpr.specialize() can fold a
        # ``dataset_index`` leaf living under a rank↔feature reshape.
        return ReshapeRankNode(new_children[0], target_rank=self._target_rank)
