...
class DerivativeNode(BaseOpNode):
    ...
    **Interacting case** (particles_input=True):
    ...
    def with_children(self, new_children):
        # Rebuild around the (possibly specialized) child, preserving the
        # derivative config so StateExpr.specialize() can fold a
        # ``dataset_index`` leaf living under a gradient (e.g. force = ∂ₓU).
        return DerivativeNode(
            new_children[0],
            var=self.var,
            same_particle=self.same_particle,
            mode=self.mode,
        )
