...
.. list-table::
   ...
   * - Non-uniform sampling (irregular ``t`` or per-step ``dt``)
     - parametric
     - per-interval flow steps and process covariances — no
...
- a **PSF** (*parametric state function*) — any differentiable force model
  ...
  This takes the *damped* Gauss–Newton route (keeping the
  errors-in-variables instrument active) and **needs an initial guess**
  for the parameters, since a general nonlinear fit will not converge
  from an arbitrary start.  On the overdamped engine an
  :term:`L-BFGS` route can be forced with ``inner="lbfgs"`` (raise
  ``inner_maxiter`` for NN-scale families):
...
A note on large :math:`\Delta t`: raise ``n_substeps`` to 2 when the
dynamics is fast on the observation interval (the flow model saturates at
``n_substeps=2``), and on (near-)clean data pass ``eiv=False`` — at coarse
sampling the noise profile cannot separate a :math:`\Delta t`-independent
:math:`\Lambda` from the process noise and can fit a spurious one.  When
both ``D=`` and ``Lambda=`` are given they are held fixed and the
estimation step is skipped — the fast path when the noise levels are
already known.  See :doc:`/inference/noise_and_sampling` for when these
effects matter and how to diagnose them.
...
Both engines can estimate the dissipation of the observed dynamics with
``compute_entropy_production()`` — after ``infer_force_linear()`` (any
preset), and optionally after ``sparsify_force()``.
...
.. code-block:: python
   ...
   print(f"Sdot = {out['Sdot']:.3g} +- {out['Sdot_error']:.3g} kB/time "
         f"(fluctuation bias {out['Sdot_bias']:.3g})")

The estimator projects the phase-space velocity (mean local velocity)
:math:`v = F - D\nabla\ln\rho` onto the force basis and contracts it with
the inverse average diffusion — :math:`\Delta\hat S = 2\,M_v^\top G^{-1}
M_v` in units of :math:`k_B` (Frishman & Ronceray 2020; see
:doc:`/physics_reference` for the formulas).  The totals ``DeltaS`` /
``error_DeltaS`` also appear in
:meth:`~SFI.inference.OverdampedLangevinInference.print_report` and
:meth:`~SFI.inference.OverdampedLangevinInference.report_dict`.

Interpretation and caveats:

- **Lower bound.**  The projection sees only the currents resolved by the
  basis in the *observed* phase space: hidden degrees of freedom and
  coarse-graining can only decrease the estimate (up to the reported
  :math:`+2 N_b/\tau_N` fluctuation bias, which is not subtracted).
- **Detection floor.**  For detailed-balance dynamics the estimate is
  consistent with zero within error bars; to *detect* irreversibility you
  must accumulate a few :math:`k_B` of dissipated entropy over the
  trajectory.
- **Constant diffusion.**  The contraction uses the average :math:`\bar D`;
  a warning is emitted when a state-dependent diffusion field was fitted.
- **Measurement noise.**  Localization noise adds a positive
  :math:`O(\Lambda/\Delta t)` bias not included in the error bar (a warning
  is emitted when the estimated :math:`\mathrm{Tr}\,\Lambda` is
  non-negligible relative to :math:`\mathrm{Tr}\,\bar D\,\Delta t`).

**Underdamped**
(:meth:`~SFI.inference.UnderdampedLangevinInference.compute_entropy_production`):
the same call evaluates the Stratonovich log path-probability ratio with
the fitted acceleration field split into its time-reversal even/odd
parts in the reconstructed velocity — no velocity measurements needed.
:meth:`~SFI.inference.UnderdampedLangevinInference.time_reversal_split`
returns the reversible / irreversible parts :math:`(\hat F^{+}, \hat
F^{-})` of the fit as callables.  Underdamped-specific caveats: the
sampling must resolve the velocity correlation time
(:math:`\hat\gamma\,\Delta t \lesssim 0.5`, warned otherwise); error
bars are set by the odd-sector fluctuation scale (friction), reported
via an analytic formula and an empirical block-variance estimate; the
same-sample plug-in bias is :math:`O(1/\tau_N)` and can be removed by
cross-fitting through the ``coefficients`` argument.  The bars are
**provisional** pending the full underdamped bias derivation.

See :doc:`/inference/stochastic_thermodynamics` for the physics context
and :doc:`/gallery/entropy_production_demo` for a worked example (a
driven optical trap).
