...
This page presents the mathematical foundations of the parametric
estimators used by
...
The parametric estimator addresses all three limitations.  It replaces
the finite-difference velocity by an *exact numerical flow*, accounts
for measurement noise through a *joint Gaussian likelihood* evaluated
exactly, and iterates via *Gauss–Newton* to handle nonlinear parameter
dependence.
...
:math:`\Delta t_k` — solved with :math:`n_{\text{sub}}` RK4 micro-steps
:math:`h_k = \Delta t_k / n_{\text{sub}}` (default
:math:`n_{\text{sub}} = 1`, a single 4th-order step per observation
interval) — and model the stochastic part as a **single Gaussian
increment** accumulated over that interval.  There is no operator
splitting: one deterministic flow step and one noise increment per
observation.  The step :math:`\Delta t_k` is *per interval*: datasets
sampled non-uniformly (a time vector ``t`` or a per-step ``dt`` array)
are handled by exactly the same construction, with no interpolation or
resampling.
...
.. math::
   ...
   \Phi(x;\,\theta) = z(\Delta t_k) - x, \qquad
...
where the *process-noise covariance* :math:`V_t` is obtained by
integrating the **Lyapunov equation** along the same RK4 stages as the
flow:

.. math::
   :label: process-cov

   \dot{Q} = J\,Q + Q\,J^{\!\top} + 2\,D(z), \qquad
   Q(0) = 0, \qquad V_t = Q(\Delta t_k),

with :math:`J` the running flow Jacobian.  This is exact for linear
drift and :math:`O((k\Delta t)^2)`-accurate in general — replacing the
endpoint-trapezoid approximation
:math:`V_t \approx \Delta t\,(J_t D J_t^{\!\top} + D)` of earlier
versions, which is recovered with ``SFI_EXACT_UPGRADES=0``.  The
improvement matters chiefly for diffusion estimates at coarse sampling
(:math:`k\,\Delta t \gtrsim 0.1`, with :math:`k` a typical drift
gradient); force estimates are insensitive to it.

In the leading-order approximation used by the Gauss–Newton solver, the
log-determinant contribution of :math:`V_t` is treated as
:math:`\theta`-independent.  This is justified because the
...
where :math:`\Sigma` is the block-banded matrix with blocks
:math:`(A_t, C_t)`.  Both the log-determinant and the quadratic form
are evaluated **exactly** by a reverse-time block-LDLᵀ **innovations
recursion**: sweeping the band once produces whitened residuals
:math:`\tilde r_t = L_t^{-1}(r_t - \text{carry}_t)` whose squared norm
is the quadratic form and whose factor log-determinants sum to
:math:`\log\det\Sigma` — :math:`O(T d^3)` time, one
``jax.lax.scan`` pass, no truncation and no tuning knobs.  The same
sweep whitens the parameter sensitivities, so the normal equations
below are assembled against the *exact* precision.  Masked frames and
tracking gaps simply restart the recursion: only ``bandwidth``
residuals are lost per gap.
...
nonlinearly through the flow map :math:`\Phi`.  The Gauss–Newton
solver needs the **test functions** (parameter sensitivities)
...
These are computed by an exact **per-stage θ-recursion**: the RK4
combination rules are applied to the stage values of :math:`F`,
:math:`\partial F/\partial x` and :math:`\partial F/\partial \theta`,
propagating :math:`\psi` alongside the flow.  This gives the same
:math:`\psi` as forward-mode autodiff at the cost of *one* force /
Jacobian / parameter-gradient evaluation per stage — instead of
:math:`n_{\rm params}` forward tangents through the integrator — with a
zero-autodiff fast path for linear-in-θ bases.  It also removes the
second-order-autodiff compile wall for bases containing derivatives of
fitted fields.
...
under noise (``eiv=True``, the default; ``eiv=False`` recovers the
plain MLE).

.. note::

   **Coarse sampling, (near-)clean data.**  A Δt-independent
   measurement noise :math:`\Lambda` is identifiable against process
   noise :math:`\propto \Delta t` only when the sampling resolves the
   dynamics.  At coarse effective sampling on nonlinear systems the
   ``(D, Λ)`` profile can fit a *spurious* :math:`\Lambda` on clean
   data, degrading :math:`\hat D` and the force fit.  ``eiv="auto"``
   does not inspect the noise level — when you know the data is clean
   and the sampling is coarse, pass ``eiv=False`` (which also holds
   :math:`\Lambda \equiv 0` in the profile) and use
   ``n_substeps=2``.
...
.. math::
   ...
   G_{\alpha\beta}
   &= \sum_t
      \tilde\psi_\alpha^{\!\top} \, \tilde\psi_\beta, \\
   f_\alpha
   &= \sum_t
      \tilde\psi_\alpha^{\!\top} \, \tilde\rho_t,

where :math:`\rho_t = r_t + \psi \cdot \theta` is the augmented
residual and the tildes denote the exact innovations whitening of
:eq:`nll` (see :ref:`parametric-algorithm` for the assembly).
...
.. list-table::
   ...
   * - Cost per iteration
     - :math:`O(T n^2 d)`
     - :math:`O(T n d^3)` (banded)
   * - Iterations needed
     - 1 (exact solve)
     - 2–5 GN steps

The parametric approach is **more accurate at larger**
...
1. **Moment init:** :math:`(\mathbf{D}, \Lambda)` from closed-form
   ...
   Both stencils are exactly unbiased under non-uniform sampling
   (per-sample two-interval normalisation).
2. **Gauss–Newton loop:** iterated whitened least-squares at the
   fixed noise levels.
3. **Reprofile:** one exact banded-NLL refinement of
   :math:`(\mathbf{D}, \Lambda)` at the fitted :math:`\theta`, run on
   **cached fixed-θ tensors** (the flow residuals and stage Jacobians
   are computed once; each L-BFGS iteration of the profile costs
   linear algebra only — zero basis evaluations), followed by a warm
   re-solve (the iteratively-reweighted fixed point for
   linear-in-:math:`\theta` models).
...
positions are observed, so the velocity at each residual is resolved by
...
noise enters the position at :math:`O(\Delta t^3)` — which makes the
Lyapunov-exact covariance :eq:`process-cov` mandatory for non-uniform
sampling (the leading-order blocks assume equal neighbouring
intervals).
...
Three refinements of the covariance blocks and estimating equation ship
default-on inside the exact core (disable with
``SFI_EXACT_UPGRADES=0``):

- **Lyapunov-exact process covariance** — :eq:`process-cov` above.
- **Convexity correction** — measurement noise induces a residual-mean
  bias :math:`\tfrac12 \nabla^2\Phi : \Lambda` through drift curvature;
  the correction cancels it (active on the noise/EIV path,
  non-interacting overdamped models).
- **Huberized whitened score** — an internal bounded-influence variant
  of the estimating equation for outlier-contaminated data.
...
:meth:`~SFI.inference.OverdampedLangevinInference.infer_diffusion` fits
a state-dependent diffusion tensor :math:`D(x;\,\theta_D)` by
optimising the exact banded NLL with the force held fixed, on the same
cached fixed-θ tensors as the profile: per-point process covariances
for any candidate :math:`D` are rebuilt from the cached RK4 stage
Jacobians (:eq:`process-cov`), so each L-BFGS iteration costs linear
algebra only — zero force or basis evaluations.
...
The optimisation is initialised by projecting the moment-estimated
...
3. Brückner, D. B., Ronceray, P. & Broedersz, C. P., *Inferring the
   ...
   125, 058103 (2020).
   — Underdamped Langevin Inference (ULI), extended here to the
