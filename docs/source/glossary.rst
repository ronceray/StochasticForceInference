...
.. glossary::
   ...
   parametric estimators
      ...
      more RK4 flow steps per observation interval, an exact
      :term:`banded-innovations` NLL,
      native :math:`(\mathbf{D}, \Lambda)` profiling; robust to
      measurement noise and coarse sampling, accepts nonlinear-in-θ
      models and non-uniform sampling.  See
   ...
   banded-innovations
      The exact evaluation scheme of the parametric NLL: the residual
      covariance is block-banded (measurement noise correlates
      consecutive residuals), and a reverse-time block-LDLᵀ innovations
      recursion whitens residuals and sensitivities exactly — no
      truncation, no tuning knobs.

   conditional NLL
      The negative log-likelihood seen as a function of
      :math:`(\mathbf{D}, \Lambda)` with the model parameters
      :math:`\theta` held at their fitted values — minimised once (on
      cached fixed-θ tensors) to refine the profiled noise levels.
