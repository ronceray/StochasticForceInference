...
# Canonical tag vocabulary — one entry per tag in use, colour = category.
# Consolidation (2026-07): pastis/sparsification -> sparsity, spde -> SPDE,
# diffusion-field folded into multiplicative-noise, interactions folded into
# multi-particle; the ambiguous "experimental"/"experimental-workflow"
# maturity flags were dropped.  Keep this dict in sync with the
# ``# sphinx_gallery_tags`` lines of ``examples/gallery/**/*.py``.
TAG_COLORS: dict[str, str] = {
    ...
    "linear": "#0dcaf0",
    "nonlinear": "#fd7e14",
    "neural-network": "#d63384",
    # model selection, validation & tooling
    "sparsity": "#ffc107",
    "benchmark": "#dc3545",
    "diagnostics": "#495057",
    "solver-comparison": "#868e96",
    # data / workflow features
    "multi-particle": "#20c997",
    "per-particle": "#20c997",
    "multi-experiment": "#17a2b8",
    "custom-basis": "#17a2b8",
    "extras": "#17a2b8",
    "time-dependent": "#17a2b8",
    "large-scale": "#17a2b8",
    # noise structure
    "multiplicative-noise": "#6f42c1",
    "anisotropic": "#6f42c1",
    # physics
    "non-equilibrium": "#e66100",
    "nonreciprocal": "#e66100",
    "thermodynamics": "#b02a37",
    "ecology": "#2f9e44",
    "reaction-diffusion": "#2f9e44",
    # system dimension
    "1D": "#adb5bd",
    "2D": "#adb5bd",
    "3D": "#adb5bd",
...
def _inject_tags_into_gallery(app: Sphinx, doctree, docname: str) -> None:
    ...
    if sg_conf:
        for examples_dir, gallery_dir in zip(
            ...
            gdir = gallery_dir.rstrip("/")
            if gdir in docname:
                # Map docname -> source path, PRESERVING subdirectories
                # (gallery/advanced/foo_demo -> examples/gallery/advanced/
                # foo_demo.py); a bare basename would silently skip every
                # demo living in a gallery subsection.
                rel = docname.split(gdir + "/", 1)[-1]
                candidate = Path(examples_dir) / (rel + ".py")
                if candidate.exists():
                    source_path = candidate
                    break

    if source_path is None:
        # No sphinx-gallery conf (e.g. RTD builds serve the committed
        # pages without gen_gallery): fall back to the repo layout.
        rel = docname.split("gallery/", 1)[-1] if "gallery/" in docname else None
        if rel:
            candidate = (
                Path(app.srcdir).parents[1] / "examples" / "gallery" / (rel + ".py")
            )
            if candidate.exists():
                source_path = candidate
    if source_path is None:
