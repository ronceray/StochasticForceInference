...
*Source:* :mod:`SFI.inference.parametric_core` — ``banded_nll_gram``

The single-step RK4 flow residuals :math:`r_k = x_{k+1} -
\Phi(x_k;\theta)` carry a block-banded covariance :math:`\Sigma`
(overdamped: block-tridiagonal; underdamped: pentadiagonal).  The block
LDLᵀ factorisation :math:`\Sigma = L\,S\,L^\top` has a *banded*
unit-lower factor, so the innovations

.. math::

   e_m = r_m - L^{(1)}_m e_{m-1} \;[-\, L^{(2)}_m e_{m-2}],
   \qquad
   \tilde e_m = R_m^{-1} e_m,
   \quad R_m = \operatorname{chol} S_m,

whiten the residual stream **exactly** in one sequential pass, and

.. math::

   -2\log L(\theta, \mathbf{D}, \Lambda)
   = \sum_m \tilde e_m^\top \tilde e_m
   \;+\; 2 \sum_m \log \det R_m \;+\; \mathrm{const}.

Whitening the regressor columns alongside yields the exact GLS Gram and
moments as plain sums — no overlapping windows, no truncated precision.
The default reverse-time (anti-causal) recursion keeps every
errors-in-variables instrument–residual pairing measurement-noise-clean.
Full construction: :doc:`/inference/parametric_algorithm`.
...
In the estimator's extensive normalization — :math:`G_{ij} = \sum_t
\mathrm{d}t\;\mathbf{b}_i^\top (2\bar{\mathbf{D}})^{-1}\mathbf{b}_j`,
:math:`M_i = \sum_t \mathrm{d}\mathbf{x}_t^\top
(2\bar{\mathbf{D}})^{-1}\mathbf{b}_i` (the fit :math:`G\hat F = M` is
invariant under this rescaling) — the Gram is the Hessian of the
Onsager–Machlup action, hence exactly the Fisher information of the
coefficients:

.. math::

   \operatorname{Cov}(\hat{F}) = G^{-1},
   \qquad
   I_F = \tfrac{1}{2}\,\hat{F}^\top M,
   \qquad
   \text{NMSE}_{F,\text{pred}}
   = \frac{\operatorname{Tr}\bigl(G\cdot\operatorname{Cov}(\hat{F})\bigr)}{2\,I_F}
   = \frac{N_b}{2\,I_F}
