...
def test_force_core_tikhonov_bounds_rank_deficient_gram():
    ...
    # the frozen-precision IRLS path (inner="lbfgs") carries the Tikhonov
    # condition-cap ridge; PSF-auto would resolve to damped GN, so request
    # L-BFGS explicitly to exercise the ridge on the exact core.
    res = solve_force_od(coll, _degenerate_psf(), n_substeps=4,
                         max_outer=3, inner="lbfgs")
...
def test_gn_line_search_bar_uses_merit_objective():
    ...
    nll (a differently-normalised conditional NLL), comparing trial-merit
    against gram-nll can
