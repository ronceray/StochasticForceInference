...
def test_od_force_minimal_psf_loss_path():
    ...
    res = solve_force_od(coll, _linear_drift_psf(2), max_outer=6)
    # PSFs resolve to damped GN on the (default) exact core — the exact
    # Gram is PSF-capable and keeps the EIV instrument active
    assert res.info.get("inner") == "gn"
