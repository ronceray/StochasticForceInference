...
"""
ABP (Active Brownian Particle) model building blocks — test-local.

Mirror of :mod:`examples._gallery_utils.abp`; kept separate so tests do
not import from the examples tree.  Composed entirely from the geometric
primitives in :mod:`SFI.bases.pairs` (``heading_vector``,
``pair_direction``, ``angle_coupling``, ``parametric_radial_kernel``)
using the ``*`` / ``+`` Interactor algebra — no direct call to
